...
function agileBacklog() {
    ...
    $wb.each(function() {
        ...
        if ($.inArray(bug.status, closed_statuses) > -1) {
            ...
            var assigned_to = $(this).siblings('.bz_assigned_to_column').text();
            if (assigned_to.trim() == 'nobody@mozilla.org') {
                open_stories++;
    ...
    var $tr = $('<tr>'),
        ...
        $componentsGraph = $('<div>',
            {'id': 'componentsGraph', 'class': 'graph'}),
    ...
    $text.append($('<br>'));
    $text.append($('<span>',
        {'text': 'Assigned Stories: ' + assigned_stories}));
    $text.append($('<br>'));
    ...
    var plot_config = {series: {pie: {show: true}}, legend: {show: false}};
    $.plot($('#pointsGraph'), points_data, plot_config);
    $.plot($('#componentsGraph'), components_data, plot_config);
    $.plot($('#usersGraph'), users_data, plot_config);
